...
Finally, it is possible that the cross-linguistic regularity is due primarily to languages in a particular language family. To test this possibility, we included language family information from the World Atlas of Language Structures database \cite<WALS,>{haspelmath2005world}. We constructed a mixed effect model  with complexity ratings as fixed effect predictor of length in terms of number of characters. The random-effect structure included language family as both random slopes and intercepts. There was a reliable effect of complexity on length ($\beta = 1.26$, $t = 6.36$), suggesting that the complexity bias is general characteristic of human languages. 
