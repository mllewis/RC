...
# plot
geon_norms_plot = ggplot(cg_norms, aes(y=meanRating, x=jitter(obj_class))) +
...
# plot
geon_control_plot = qplot(len,p_complex, position=position_dodge(),
...
```{r include = F}
#### GEON PLOTS 
png("/Documents/GRADUATE_SCHOOL/Projects/ref_complex/Papers/RC/writeup/long_form/figures/study1_plots.png", height = 5, width = 15, units = "in", res=500)
multiplot(geon_norms_plot, geon_c_plot, geon_control_plot, cols = 3)
dev.off()
```
...
objs_norms_plot = qplot(rating_1,rating_2, position=position_dodge(),
...
```{r, warning = F, echo = F, fig.height = ps2, fig.width = 2*ps2}
### STUDY 3 PLOTS
png("/Documents/GRADUATE_SCHOOL/Projects/ref_complex/Papers/RC/writeup/long_form/figures/study3_plots.png", height = 6, width = 12, units = "in", res=500)
multiplot(geon_rt_plot, obj_rt_plot, cols = 2)
dev.off()
```
...
#plot
obj_control_plot = qplot(len,p_complex, position=position_dodge(),
      data=ms,geom="line",ylab="Prop. selection complex object", 
      xlab="Number of syllables")  +
  geom_linerange(aes(ymin=ciwl,ymax=ciul), position=position_dodge(.9)) +
  geom_point(aes(len,p_complex), position=position_dodge(.9)) +
...
```{r include = F}
#### Obj PLOTS 
png("/Documents/GRADUATE_SCHOOL/Projects/ref_complex/Papers/RC/writeup/long_form/figures/study2_plots.png", height = 5, width = 15, units = "in", res=500)
multiplot(objs_norms_plot, obj_c_plot, obj_control_plot, cols = 3)
dev.off()
```
<a name="7"/>
...
complex = md.agg[md.agg$condition == '"complex"',"log.length"]
simple = md.agg[md.agg$condition == '"simple"',"log.length"]
t.test(complex, simple, paired = TRUE)
mean(complex)
sd(complex)
mean(simple)
sd(simple)
...
```{r}
### STUDY 4 PLOT###
png("/Documents/GRADUATE_SCHOOL/Projects/ref_complex/Papers/RC/writeup/long_form/figures/study4a_plot.png", height = 6, width = 6, units = "in", res=500)
ggplot(ms.phon, aes(y=complexity, x=jitter(mrc.phon))) +
  geom_pointrange(aes(ymax = complexity+cih,ymin=complexity-cil)) + 
  geom_smooth(method = "lm", color="blue", formula = y ~ x) +
  annotate("text", x=10, y=2, color = "red", size=rs,
           label=paste("r=",round(cor(ms.phon$complexity, ms.phon$mrc.phon), 2))) +
...
xlingplot = ggplot(c_l, aes(language, corr, fill = checked)) + 
  geom_bar(stat = "identity") + 
...
```{r include = F}
#### XLING PLOTS 
png("/Documents/GRADUATE_SCHOOL/Projects/ref_complex/Papers/RC/writeup/long_form/figures/xling_plot.png", height = 6, width = 12, units = "in", res=500)
xlingplot
dev.off()
```
