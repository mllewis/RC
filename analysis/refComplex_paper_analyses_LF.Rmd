...
processNorms = F # process norms or load norms? (process to see study time histograms)
doSlow = F # do very time-consuming pre-processing steps?
fs = 14 # set graphical params
ts = 14
rs = 5
ps1 = 3
ps2 = 4
...
# across all trials
summary(glm(as.factor(responseValue) ~ len, data=dc, family = "binomial"))
...
ggplot(both.exps, aes(x = len, y = p_complex)) +
  geom_pointrange(aes(color = exp, ymin=ciwl, ymax=ciul), size = 1) +
  geom_smooth(method = "lm", color = "black", se = F) +
  themeML +
  xlab("Number of syllables") +
  ylab("Prop. selecting complex object") +
  ggtitle("Experiments 2 and 3, 1-5 condition only") 
```
...
md$description <- str_trim(gsub(" ", "", md$description, fixed = T))#get rid of whitespace
md$length = nchar(md$description)-2
...
md.ms %>%
      group_by(condition) %>%
      summarise(mean = mean(log.length),
                  sd = sd(log.length))
```
...
# merge and aggregate
md <- md %>% 
      mutate(picture = as.numeric(picture)) %>%
      left_join(co_norms, by=c("picture"= "ratingNum")) %>%
...
## stats: relationship between length and complexity norms (continous analysis)
...
Finally, we ask whether the relationship between length and number of characters remains reliable, controling for language family. 
```{r}
...
xling.long = gather(xling, "language", "length",2:81) %>%
