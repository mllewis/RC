...
processNorms = F # process norms or load norms? (T to see study time histograms)
...
Finally, we also looked at the relationship between length and complexity controling for surprisal (Piantadosi, et al., 2011). We used bigram surprisal values from the British National Corpus (BNC) [5]. As in Piantadosi et al. (2011), we use Spearman correlations. Below we plot the correlation coefficient between length in characters and other factors, indicated along the x-axis. We look at surprisal, frequency in the BNC corpus, and complexity The symbols on the plot show the correlation, partialing out one of the other factors (where the partialed factor is indicated by the shape of the symbol). 
...
Finally, we ask whether the relationship between length and number of characters remains reliable, controling for language family using data from the WALS dataset [6].  This dataset included family data for 68 out of 80 of our langauge. Below is the by-family correlation between length and complexity across words Across all 16 families, we see a postive complexity bias.
```{r, include = F}
# read in Lupyan and Dale (2010) PLOS data
ld_plos_data = read.table("../data/corpus/LD_plos_data.txt", fill = T, 
                          header = T, sep = "\t", na.strings = "*") 

# fix language labels
ld_plos_data$ethnologue_lang_name = tolower(ld_plos_data$ethnologue_lang_name)
ld_plos_data$ethnologue_lang_name[ld_plos_data$ethnologue_lang_name == "standard german"] <- "german"
ld_plos_data$ethnologue_lang_name[ld_plos_data$ethnologue_lang_name == "tosk albanian"] <- "albanian"
ld_plos_data$ethnologue_lang_name[ld_plos_data$ethnologue_lang_name == "catalan-valencian-balear"] <- "catalan"
ld_plos_data$ethnologue_lang_name[ld_plos_data$ethnologue_lang_name == "haitian creole french"] <- "haitian.creole"
ld_plos_data$ethnologue_lang_name[ld_plos_data$ethnologue_lang_name == "irish gaelic"] <- "irish"
ld_plos_data$ethnologue_lang_name[ld_plos_data$ethnologue_lang_name == "central khmer"] <- "khmer"
...
ld_plos_data = ld_plos_data[-which(grepl("sign language", ld_plos_data$ethnologue_lang_name)),]

ld_plos_data$lang2= ifelse(grepl("arabic", ld_plos_data$ethnologue_lang_name), "arabic",
                   ifelse(grepl("azerbaijani", ld_plos_data$ethnologue_lang_name), "azerbaijani",
                          ifelse(grepl("chinese", ld_plos_data$ethnologue_lang_name), "chinese",
                                 ifelse(grepl("hmong", ld_plos_data$ethnologue_lang_name), "hmong",
                                        ifelse(grepl("mongolian", ld_plos_data$ethnologue_lang_name), "mongolian",
                                               ifelse(grepl("yiddish", ld_plos_data$ethnologue_lang_name), "yiddish",
                                                      ld_plos_data$ethnologue_lang_name))))))

```

```{r, include = F}
# Get language data in long form
ld_plos_data.fam = select(ld_plos_data, langFamily, nativeCountry, lang2) %>%
  distinct(lang2)
...
xling.long = left_join(xling.long, ld_plos_data.fam, by = c("language" = "lang2")) 
```
...
Following Jaeger, et al. (2011) [7], we included random intercepts and slopes by-family and by-native country. We built a model predicting length in terms of number of phonemes with complexity and frequency as fixed effects. The effect of complexity on length remained reliable in this model.  Model parameters are presented below. 

```{r echo = F, fig.width = 6, results = 'asis' }

#full = summary(lmer(length ~ complexity + subt.log.freq + # doesn't converge
...
[5] Clear J. (1993). The British National Corpus (MIT Press, Cambridge, MA).

[6] Haspelmath, M., Dryer, M. S., Gil, D., & Comrie, B. (2005). The world atlas of language structures.

[7] Jaeger, T. F., Graff, P., Croft, W., & Pontillo, D. (2011). Mixed effect models for genetic and areal dependencies in linguistic typology. Linguistic Typology, 15, 281–320.