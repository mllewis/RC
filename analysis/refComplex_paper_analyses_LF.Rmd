...
both.exps = rbind(exp2, ms)
both.exps$exp = as.factor(c(2,2,3,3,3))

ggplot(both.exps, aes(x = len, y = p_complex, size = n, color = exp)) +
  geom_point()

# No evidence for effect of stimuli
```
...
# means and sds
mean(md.agg[md.agg$condition == '"complex"',"log.length"])
sd(md.agg[md.agg$condition == '"complex"',"log.length"])
mean(md.agg[md.agg$condition == '"simple"',"log.length"])
sd(md.agg[md.agg$condition == '"simple"',"log.length"])
```
...
cp_obj = ggplot(ms_obj.phon, aes(y = complexity, 
                                 ...
                                 group = obj)) +
  geom_pointrange(aes(ymax = complexity+cih,ymin=complexity-cil)) + 
  geom_smooth(method = "lm", formula = y ~ x, aes(color = obj)) +
  xlab("# of phonemes") +
  ylab("Complexity rating") +
...
summary(lm(complexity~ mrc.phon + as.factor(obj) + subt.log.freq, eng))
```
