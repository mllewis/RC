---
title: The length of words reflects their conceptual complexity (Supplementary Information)
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true
...
This document was created from an R Markdown file. The R Markdown file can be found <a href="https://mllewis.github.io/projects/RC/RCSI.html" target="_blank"> here</a>. All analyses and plots can be reproduced from the <a href="https://github.com/mllewis/RC/tree/master/data" target="_blank">raw data</a> with the code in this file. This document also contains links to the experimental tasks.
...
processNorms = T # process norms or load norms? (T to see study time histograms)
...
opts_chunk$set(echo=F, message=F, warning=F, error=F, cache=F)
```
...
The task can be found <a href="https://mllewis.github.io/projects/RC/experiment34/refcomplex34.html" target="_blank"> here</a>.
...
```{r 1:geon_complexity_norms, warning = F, include = F}
...
mean(cg_norms$meanRating)
sd(cg_norms$meanRating)
```


## Study 2: Geon mapping task

The task can be found <a href="https://mllewis.github.io/projects/RC/experiment38/refcomplex38.html" target="_blank"> here</a>. 
...
```{r 2:geon_mapping, warning=F, include=F}
...
The task can be found <a href="https://mllewis.github.io/projects/RC/experiment40/refcomplex40.html" target="_blank"> here</a>.
...
# get participant-wise means
p.means = dc %>%
  group_by(workerid,len) %>%
  summarise (n_complex = sum(responseValue == "\"complex\"") ,
             n = n()) %>%
  mutate(prop_complex = n_complex/n)
```

```{r, warning = F, message = F, echo = F, fig.width = ps1, fig.height = ps1}
# get condition-wise means
ms = p.means %>%
  group_by(len) %>%
  multi_boot_standard(column = "prop_complex")

# plot
qplot(len,mean, position=position_dodge(),
  ...
  geom_point(aes(len,mean), position=position_dodge(.9)) +
...
# across all trials
summary(glmer(responseValue ~ len + (len|workerid), data=dc, family = "binomial"))

# across participant means
summary(lm(prop_complex ~ len, data=p.means))
```
...
```{r, warning = F, message = F, echo = F, fig.width = ps1+1, fig.height = ps1}
# experiment 2
exp2 = dc2 %>%
  filter(objCondition ==  "\"1-5\"") %>%
  ddply(. ,.(langCondition), function (d, dv) {p.fc(d,"\"complex\"")}) %>%
  rename(len = langCondition) %>%
  select(-n)
exp2$len = as.numeric(c(4, 2))

# experiment 3
exp3 = ms %>%
  rename(p_complex = mean, ciwl = ci_lower , ciul = ci_upper )

both.exps = rbind(exp2, exp3)
both.exps$exp = as.factor(c(2,2,3,3,3))

ggplot(both.exps, aes(x = len, y = p_complex)) +
  geom_pointrange(aes(color = exp, ymin=ciwl, ymax=ciul)) +
  geom_smooth(method = "lm", color = "black", se = F) +
  themeML +
  xlab("Number of syllables") +
  ylab("Prop. selecting complex object") +
...
mean(co_norms$rating_1)
sd(co_norms$rating_1)
mean(co_norms$rating_2)
sd(co_norms$rating_2)

# mean difference
mean(abs(co_norms$rating_1 - co_norms$rating_2))
```

## Study 5: Real object mapping task 

The task can be found <a href="https://mllewis.github.io/projects/RC/experiment35/refcomplex35.html" target="_blank"> here</a>.
...
The task can be found <a href="https://mllewis.github.io/projects/RC/experiment41/refcomplex41.html" target="_blank"> here</a>.
...
summary(glmer(responseValue ~ len + (1|workerid), data=dc, family = "binomial", control=glmerControl(optimizer = "bobyqa")))

```

## Study 7: Real object production task

The task can be found <a href="https://mllewis.github.io/projects/RC/experiment27/refcomplex27.html" target="_blank"> here</a>.
...
pictures <- d %>%
            select(contains("pic"), workerid) %>%
            gather("trial", "picture", contains("pic")) %>%
            separate(trial, c("temp", "trial"), sep = "_") %>%
            select(-temp)
            
descriptions <- d %>%
              select(contains("desc_"),workerid) %>%
              gather("trial", "description",contains("desc_") ) %>%
              separate(trial, c("temp", "trial"), sep = "_") %>%
              select(-temp)

conditions <- d %>%
              select(contains("cond"),workerid) %>%
              gather("trial", "condition",contains("cond") ) %>%
              separate(trial, c("temp", "trial"), sep = "_") %>%
              select(-temp)

md = full_join(pictures, descriptions) %>%
     full_join(conditions) %>%
     left_join(co_norms[,c("meanRating", "ratingNum")], by=c("picture" = "ratingNum")) %>%
     rename(c.norms = meanRating) %>%
     mutate(numWords = sapply(gregexpr("\\W+", description), length) - 1,
            description = str_trim(gsub(" ", "", description, fixed = T)),
            length = nchar(description) - 2,
            trial = as.numeric(trial),
            picture = as.numeric(picture),
...
md %>%
  group_by(condition) %>%
  summarise(log.mean = mean(log.length),
            log.sd = sd(log.length),
            mean = mean(length),
            sd = sd(length))
```
...
# plot
ggplot(md, aes(c.norms,log.length)) +
  geom_point() + 
  geom_smooth(method = "lm", color="blue", formula = y ~ x) +
...
# paired t-test of length between simple and complex conditions 
t.test(complex[,"log.length"], simple[,"log.length"], paired = T)
summary(lmer(log.length ~ condition + (condition|workerid), md))

## stats: relationship between length and complexity norms (continuous analysis)
cor.test(md$log.length, md$c.norms)
summary(lmer(log.length ~ c.norms + (c.norms|workerid), md)) # this is the reported analyis
```


## Study 8a: Geon study time task 

The task can be found <a href="https://mllewis.github.io/projects/RC/experiment37/refcomplex37.html" target="_blank"> here</a>.
...
if (processNorms) {
  ...
  raw = raw[raw$Answer.correct > 20, ] # excludes 9 participants
  ...
  # mean and sd by participant (in s for interpretability)
  mean(d$Answer.train_rt) * .001
  sd(d$Answer.train_rt) * .001
}
```
...
if (processNorms) {
  ...
  ## stats: relationship between study times and memory performance
  # paired t-test
  ...
  # rt by item for in paper (in seconds for interpretability)
  d %>%
  group_by(obj) %>%
  summarise(mean = mean(Answer.train_rt)) %>%
  summarise(grand.mean = mean(mean) * .001,
            grand.sd = sd(mean) * .001)
...
The task can be found <a href="https://mllewis.github.io/projects/RC/experiment30/refcomplex30.html" target="_blank"> here</a>.
...
if (processNorms){
  ...
  # look at accuracy, and exclude those below chance
  # boxplot(raw$Answer.correct, main = "distribution before accuracy exclusions")
  raw = raw[raw$Answer.correct > 30, ] # excludes 6 participants
  ...
  # mean and sd by participant (in seconds for interpretability)
  mean(d$Answer.train_rt) * .001
  sd(d$Answer.train_rt) * .001
}
```
...
if (processNorms){
  ...
  # rt by item for interpretability in paper
  d %>%
  filter(Answer.train_image != 61, Answer.train_image != 62) %>%
  group_by(Answer.train_image) %>%
  summarise(mean = mean(Answer.train_rt)) %>%
  summarise(grand.mean = mean(mean) * .001,
            grand.sd = sd(mean) * .001)
...
The task can be found <a href="https://mllewis.github.io/projects/RC/experiment26/refcomplex26.html" target="_blank"> here</a>.
...
if (processNorms) {
  ...
  # remove subjects who participated in two different hits of this experiment
  ...
  md$obj <- class$objects_MLL[index]
...
p_char = ggplot(ms.chars2, aes(x=nchars)) + 
    geom_histogram(fill = "black", alpha = .6 , binwidth = 1, origin = -0.5) +
  ylab("Frequency") +
  xlab("Word length (characters)") +
  ggtitle("Word lengths") +
  scale_x_continuous(breaks = seq(2,12,2))  +
  annotate("text", x=10, y=100, color = "red", size = rs,
    label=paste("n=",dim(ms.chars2)[1])) +
  themeML


multiplot(p_char, p_phon, p_syl, p_morph, cols = 2)
...
```{r, echo = F, fig.width = 2.5*ps1, fig.height = 2*ps1}
## plot complexity norms vs. word length (phonemes) for open class, low concreteness, monomorphemic, and object labels only
...
## monomorphemic
# aggregate
ms_mono.phon = aggregate(complexity ~ word + mrc.phon, 
                         data=eng[eng$clx.morph == 1,], mean)
...
## object labels only
# aggregate
ms_obj.phon = aggregate(complexity ~ word + mrc.phon + obj, 
                         data=eng, mean)

# get CIs
ms_obj.phon$cih <- aggregate(complexity ~ word + mrc.phon + obj, 
                              data=eng, ci.high)$complexity
ms_obj.phon$cil <- aggregate(complexity ~ word + mrc.phon + obj, 
                              data=eng, ci.low)$complexity
ms_obj.phon$obj = as.factor(ms_obj.phon$obj)
ms_obj.phon$obj2 = factor(ms_obj.phon$obj, labels = c("non-object", "object"))

cp_obj = ggplot(ms_obj.phon, aes(y = complexity, 
                                 x = jitter(mrc.phon), 
                                 group = obj)) +
  geom_pointrange(aes(ymax = complexity+cih,ymin=complexity-cil)) + 
  geom_smooth(method = "lm", formula = y ~ x, aes(color = obj2)) +
  xlab("# of phonemes") +
  ylab("Complexity rating") +
  ggtitle("Object labels:\nComplexity Rating vs. # of phonemes") +
  themeML +
  theme(legend.title=element_blank())

# what does it look like for non objects? Factor by object
multiplot(cp_o, cp_c, cp_m, cp_obj, cols = 2)
```

```{r, include = F}
## stats: correlations between length and complexity in English - all words and subsets
# mean and sd
mean(ms.phon$complexity)
sd(ms.phon$complexity)
...
## SUBSET 4: object labels
ms_obj.syl = aggregate(complexity ~ word + mrc.syl + obj, data=eng, mean)
ms_obj.phon = aggregate(complexity ~ word + mrc.phon + obj, data=eng, mean)
ms_obj.morph = aggregate(complexity ~ word + clx.morph + obj, data=eng, mean)

# correlation between syllables and complexity [obj vs. non-obj]
cor.test(ms_obj.syl[ms_obj.syl$obj == 1, "complexity"], ms_obj.syl[ms_obj.syl$obj == 1, "mrc.syl"])
cor.test(ms_obj.syl[ms_obj.syl$obj == 0, "complexity"], ms_obj.syl[ms_obj.syl$obj == 0, "mrc.syl"])

# correlation between phonemes and complexity [obj vs. non-obj]
cor.test(ms_obj.phon[ms_obj.phon$obj == 1, "complexity"], ms_obj.phon[ms_obj.phon$obj == 1, "mrc.phon"])
cor.test(ms_obj.phon[ms_obj.phon$obj == 0, "complexity"], ms_obj.phon[ms_obj.phon$obj == 0, "mrc.phon"])

# correlation between morphemes and complexity [obj vs. non-obj]
cor.test(ms_obj.morph[ms_obj.morph$obj == 1, "complexity"], ms_obj.morph[ms_obj.morph$obj == 1, "clx.morph"])
cor.test(ms_obj.morph[ms_obj.morph$obj == 0, "complexity"], ms_obj.morph[ms_obj.morph$obj == 0, "clx.morph"])

# partial out frequency
ms_obj.freq.phon.obj = aggregate(complexity ~ word + mrc.phon + subt.log.freq, data=eng[eng$obj == 1,], mean)
pcor.test(ms_obj.freq.phon.obj$complexity,ms_obj.freq.phon.obj$mrc.phon,
          ms_obj.freq.phon.obj$subt.log.freq)

summary(lm(mrc.phon~ complexity*as.factor(obj), eng))
```
...
```{r, results = 'asis', echo = F, warning = F, fig.width = 6}
...
full.piantadosi.df$corr = c("freq.bnc", "surprisal")
...
full.partials = data.frame(p.corrs = c(full.p.cor.surp.f.bnc, full.p.cor.freq.bnc.s), 
                           corr =c("surprisal",  "freq.bnc" ),
                           partialed = c("freq.bnc",  "surprisal"))
...
complexity.piantadosi.df$corr = c("freq.bnc", "surprisal", "complexity")
...
complexity.partials = data.frame(p.corrs = c(p.cor.surp.f.bnc,  p.cor.surp.c, p.cor.freq.bnc.s,
                                             p.cor.freq.bnc.c, p.cor.complex.s, p.cor.complex.f.bnc),
                                 corr = c("surprisal", "surprisal", "freq.bnc", 
                                          "freq.bnc",  "complexity", "complexity"),
                                 partialed = c("freq.bnc", "complexity", "surprisal", 
                                               "complexity",  "surprisal",  "freq.bnc"))
...
both.piantadosi.df$corr  <- factor(both.piantadosi.df$corr,  c("surprisal", "freq.bnc",  "complexity"))
both.partials.df$corr  <- factor(both.partials.df$corr, c( "surprisal",  "freq.bnc", "complexity"))

ggplot(both.piantadosi.df) +
  geom_bar(stat = "identity", aes(y = abs(estimate), x = corr, fill = corr))+
  scale_fill_manual(values = c("red", "pink", "blue"), guide = FALSE) +
...
```{r, include = F}
## stats: complexity and surprisal
cor.test(lf.data$complexity, lf.data$surprisal, method = 
           "spearman")
cor.test(lf.data$len, lf.data$surprisal, method = 
           "spearman")

## stats: complexity and length, controling for surprisal
pcor.test(lf.data$mrc.phon, lf.data$complexity, lf.data$surprisal, method = "spearman")
```
...
## stats: complexity and length, controling for surprisal and frequency
m = lm(mrc.phon ~ complexity + surprisal + log.bnc.frequency, lf.data)
...
ggplot(accuracy.df, aes(x=language, y= accuracy)) +
  geom_bar(stat = "identity", fill = "red") +
  ylim(0,1) +
  ylab("Prop. agreement") +
  xlab("Language") +
  ggtitle("Google translation accuracy") +
  geom_abline(intercept = mean(accuracy.df$accuracy), 
              slope = 0, linetype = 2) +
...
xling = read.csv("../data/corpus/xling_lens.csv") %>%
  left_join(xling.eng, by=c("ENGLISH" = "word")) %>%
  select(contains("len"), complexity, class, clx.morph, subt.log.freq) %>%
  mutate(class = as.numeric(as.character(class)), 
         clx.morph = as.numeric(as.character(clx.morph)))
```

```{r, echo = F, fig.width = 2*ps2, fig.height = ps2, include = FALSE}
...
# sort by length
xling.ML$language = reorder(xling.ML$language, xling.ML$mean)
xling.ML$language = factor(xling.ML$language, levels = rev(levels(xling.ML$language)))
...
xling.ML$checked = as.factor(ifelse(is.element(xling.ML$language, coded_languages), "yes", "no"))

ggplot(xling.ML, aes(language, mean, fill = checked)) + 
  geom_bar(stat = "identity") + 
  geom_hline(yintercept = mean(xling.ML$mean), lty=2) +
...
levels(c_l$language)[levels(c_l$language)=="ukranian"] <- "ukrainian"
```
...
ggplot(c_l, aes(language, corr, fill = checked)) + 
  ...
  geom_point(data=c_l, mapping=aes(x=language, y=p.corr.f), size=2, shape = 17) +
  ...
  geom_hline(yintercept=mean(c_l$corr), lty=2) +
...
```{r, include = F}
# read in WALS family data (taken from Lupyan and Dale (2010) PLOS)
wals_family_data = read.table("../data/corpus/wals_family_data.txt", 
                              fill = T, header = T,  na.strings = "*") 

# fix language labels
wals_family_data$ethnologue_lang_name = tolower(wals_family_data$ethnologue_lang_name)
wals_family_data$ethnologue_lang_name[wals_family_data$ethnologue_lang_name == "standard german"] <- "german"
wals_family_data$ethnologue_lang_name[wals_family_data$ethnologue_lang_name == "tosk albanian"] <- "albanian"
wals_family_data$ethnologue_lang_name[wals_family_data$ethnologue_lang_name == "catalan-valencian-balear"] <- "catalan"
wals_family_data$ethnologue_lang_name[wals_family_data$ethnologue_lang_name == "haitian creole french"] <- "haitian.creole"
wals_family_data$ethnologue_lang_name[wals_family_data$ethnologue_lang_name == "irish gaelic"] <- "irish"
wals_family_data$ethnologue_lang_name[wals_family_data$ethnologue_lang_name == "central khmer"] <- "khmer"
...
wals_family_data = wals_family_data[-which(grepl("sign language", wals_family_data$ethnologue_lang_name)),]

wals_family_data$lang2 = ifelse(grepl("arabic", wals_family_data$ethnologue_lang_name), "arabic",
                   ifelse(grepl("azerbaijani", wals_family_data$ethnologue_lang_name), "azerbaijani",
                          ifelse(grepl("chinese", wals_family_data$ethnologue_lang_name), "chinese",
                                 ifelse(grepl("hmong", wals_family_data$ethnologue_lang_name), "hmong",
                                        ifelse(grepl("mongolian", wals_family_data$ethnologue_lang_name), "mongolian",
                                               ifelse(grepl("yiddish", wals_family_data$ethnologue_lang_name), "yiddish",
                                                      wals_family_data$ethnologue_lang_name))))))

```

```{r, include = F}
# Get language data in long form
wals_family_data.fam = select(wals_family_data, langFamily, nativeCountry, lang2) %>%
  distinct(lang2)

xling.long = gather(xling, "language", "length", 1:80) %>%
              mutate(language = as.factor(tolower(lapply(str_split(language,"_"),
                                                            function(x) {x[1]})))) 
            
levels(xling.long$language)[levels(xling.long$language)=="espernto"] <- "esperanto"
levels(xling.long$language)[levels(xling.long$language)=="ukranian"] <- "ukrainian"
xling.long = left_join(xling.long, wals_family_data.fam, by = c("language" = "lang2")) 
```

```{r, include = F}
# number of missing languages [12]
xling.long %>%
  filter(is.na(langFamily)) %>%
  mutate(language = as.factor(language)) %>%
  summarise(n_missing = length(levels(language)))
# [belarusian, bosnian, cebuana, croatian, esperanto, filipino, kanada, latin, norwegian, persian, punjabi, serbian]

xling.long = filter(xling.long,!is.na(langFamily))
```

```{r, echo = F, fig.width = 2.5*ps1, fig.height = 2*ps1}
xling.fam.corrs = xling.long %>%
                  group_by(langFamily) %>%
                  do(tidy(cor.test(.$length, .$complexity)))

xling.fam.corrs$langFamily = reorder(xling.fam.corrs$langFamily, -xling.fam.corrs$estimate)
xling.ML$language = factor(xling.ML$language, levels = levels(xling.ML$language))

ggplot(xling.fam.corrs) +
  geom_bar(stat = "identity", aes(y = estimate, x = langFamily, 
                                  fill = langFamily)) +
  geom_linerange(aes(ymax = conf.high, ymin = conf.low, x = langFamily)) +
  xlab("Language Family") +
  ylab("Pearson's r between length and complexity") + 
  ggtitle("Complexity bias by language family")  +
  geom_hline(yintercept=mean(xling.fam.corrs$estimate), lty=2) +
  themeML + 
  theme(legend.position="none",
   axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5)) 
```
...
```{r echo = F, fig.width = 6, results = 'asis' }
full = lmer(length ~ complexity + subt.log.freq + 
                      (1 + complexity+subt.log.freq|langFamily) +
                      (1 + complexity+subt.log.freq|nativeCountry), 
                    data=xling.long, control = lmerControl(optimizer = "bobyqa"))

minimal = lmer(length ~ complexity + subt.log.freq + 
                      (1|langFamily) +
                      (1|nativeCountry), 
                    data=xling.long, control=lmerControl(optimizer = "bobyqa"))

print(xtable(compare(full)), type='html')

```

## Study 11: Simultaneous frequency task

The task can be found <a href="https://mllewis.github.io/projects/RC/experiment172/refcomplex172.html" target="_blank"> here</a>. Studies 11 and 12 are not reported in the paper. 
...
ggplot(ms_simult) +
  geom_bar(stat = "identity", aes(y = p_complex, 
                                  x = Answer.lang_condition, 
                                  fill = Answer.lang_condition)) +
    geom_linerange(aes(ymin=ciwl,ymax=ciul, x = Answer.lang_condition),
                   position=position_dodge(.9)) +
  geom_abline(intercept = .5, slope = 0, linetype = 2) +
  ylab("Prop. selection low freq. object") +
  xlab("Language condition") +
  ggtitle("Low freq. selections vs. language") + 
  themeML + 
...
The task can be found <a href="https://mllewis.github.io/projects/RC/experiment16/refcomplex16.html" target="_blank"> here</a>.
...
***
<h3> References </h3>
[1] Crump, M., McDonnell, J., & Gureckis, T. (2013). Evaluating Amazon's Mechanical Turk as a tool for experimental behavioral research. _PLoS ONE_ **8**.

[2] Sanchez-Meca J., Marin-Martinez, F., & Chacon-Moscoso, S. (2003). Effect-size indices for dichotomized outcomes in meta-analysis. _Psychological Methods_ **8**, 448-467.

[3] Wilson, M. (1988). MRC psycholinguistic database: Machine-usable dictionary, version 2.00. _Behavior Research Methods, Instruments, & Computers_ **20**, 6–10.

[4] Brysbaert, M., & New, B. (2009). Moving beyond Kucera and Francis: A critical evaluation of current word frequency norms and the introduction of a new and improved word frequency measure for American English. _Behavior Research Methods_ **41**, 977–990.

[5] Piantadosi, S., Tily, H., & Gibson, E. (2011b). Word lengths are optimized for efficient
communication. _Proceedings of the National Academy of Sciences_, **108(9)**, 3526–3529.
...
[8] Jaeger, T. F., Graff, P., Croft, W., & Pontillo, D. (2011). Mixed effect models for genetic and areal dependencies in linguistic typology. _Linguistic Typology_, 15, 281–320.