...
if (processNorms) {
  d <- read.csv("data/experiment/RefComplex26_all.results_A.csv",) ## THERE ARE FOUR DUPLICATE SUBJECTS-  remove these?
...
p_phon = ggplot(ms.phon, aes(x=mrc.phon)) + 
    geom_histogram(fill = "black", alpha = .6 , binwidth = 1) +
...
checksR = read.csv("data/corpus/translation_accuracy.csv")[1:500,]
checksR = checksR[checksR$ENGLISH != "peso",]

accuracy = colSums(checksR[,2:13], dims = 1)/ dim(checksR[,])[1]
accuracy = as.data.frame(accuracy)
accuracy$language = tolower(rownames(accuracy))

qplot(accuracy$language, accuracy$accuracy, fill = as.factor(accuracy),
      ylim=c(0,1), position=position_dodge(),
      data=accuracy, geom="bar", ylab="Prop. correct Google translations", 
      main = "Google translation accuracy",
      xlab="Language", stat="identity") +
  geom_abline(intercept = mean(accuracy$accuracy), slope = 0, linetype = 2) +
  theme(text = element_text(size=fs),
        plot.title=element_text(size=ts, face = "bold"),
        plot.background = element_blank(),
        panel.grid.major = element_blank(),
