...
d.fc <- function(d) {
    ...
    ci <- boot.ci(b, type = "basic")  
    ci2 = bootCI(b$t0, b$t, conf = 0.95)["basic0",]
...
p4 = ggplot(de, aes(x = rt.Mratio, y = effect_size)) +
  geom_pointrange(aes(ymax = cill_b3, ymin=ciul_b3), size = 1) +
  ggtitle("non-parametric") +
  scale_y_continuous(limits = c(-.33, 1.25)) 

multiplot(p1, p2, p3 , p4, cols = 2)

cols <- c("parametric"="black","non-param. 1.96 * se"= "blue","non-param (boot.ci)"="green", "non-param (BOOTCI basic0)" = "red")
ggplot(de, aes(x = rt.Mratio, y = effect_size)) +
  geom_pointrange(aes(x = rt.Mratio+.0005, ymax = cill, ymin=ciul,  color = "parametric"), size = 1) +
  geom_pointrange(aes(ymax = cill_b, ymin=ciul_b,  color = "non-param (boot.ci)"), size = 1) +
  geom_pointrange(aes(x = rt.Mratio-.0005, ymax = cill_b2, ymin=ciul_b2, color = "non-param. 1.96 * se"), size = 1)  +
  geom_pointrange(aes(x = rt.Mratio-.001, ymax = cill_b3, ymin=ciul_b3, color = "non-param (BOOTCI basic0)"), size = 1)  
d = dc[dc$objCondition ==  "\"1-5\"",]
```
...
246 participants completed the rating task. We excluded participants who missed a simple math problem in the middle of the task that served as an attentional check. This excluded 6 participants (2%). Complexity ratings (_M_ = 3.36, _SD_ = 1.14) were highly correlated with length. Below we plot complexity as a function of each of the three length metrics. Each point corresponds to a word. The x-coordinates have been jittered to limit over-plotting.
