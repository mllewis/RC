...
d.fc <- function(d) {
    ...
    ## non-parametrically
    d.fun <- function(u, i, lang_cond) {
          ns = table(lang_cond, u[i])
          or = (ns["\"long\"", "\"complex\""] * ns["\"short\"", "\"simple\""]) / 
              (ns["\"short\"", "\"complex\""] * ns["\"long\"", "\"simple\""])
          cf = sqrt(3)/pi
    ...
    b <- boot(d$responseValue, d.fun, R = 1000, lang_cond = d$langCondition)
    ci <- boot.ci(b, type = "basic")  
    ...
    cill_b = ci$basic[4]
    ciul_b = ci$basic[5]
    cill_b2 = b[1]$t0 - (1.96 * sd(b$t))
    ciul_b2 = b[1]$t0 + (1.96 * sd(b$t))
...
```{r, include = F}
## checking parametric vs. non-parametrics CIs
# get effect sizes and CIS
de <- ddply(dc, .(objCondition), function (d) {d.fc(d)})

# plot
p1 = ggplot(de, aes(x = rt.Mratio, y = effect_size)) +
  geom_pointrange(aes(ymax = cill, ymin=ciul), size = 1) +
  ggtitle("parametric") +
  scale_y_continuous(limits = c(-.33, 1.25)) 

p2 = ggplot(de, aes(x = rt.Mratio, y = effect_size)) +
  geom_pointrange(aes(ymax = cill_b, ymin=ciul_b), size = 1) +
  ggtitle("non-parametric (boot.ci)") +
  scale_y_continuous(limits = c(-.33, 1.25)) 

p3 = ggplot(de, aes(x = rt.Mratio, y = effect_size)) +
  geom_pointrange(aes(ymax = cill_b2, ymin=ciul_b2), size = 1) +
  ggtitle("non-parametric") +
  scale_y_continuous(limits = c(-.33, 1.25)) 

multiplot(p1, p2, p3, cols = 3)

ggplot(de, aes(x = rt.Mratio, y = effect_size)) +
  geom_pointrange(aes(x = rt.Mratio+.0005, ymax = cill, ymin=ciul), size = 1, color = "red") +
  geom_pointrange(aes(ymax = cill_b, ymin=ciul_b), size = 1, color = "blue") +
  geom_pointrange(aes(x = rt.Mratio-.0005, ymax = cill_b2, ymin=ciul_b2), size = 1, color = "green")  +
  scale_colour_manual(name="CI sources",values=c("Non-paramtric SE" = "green", "Non-parametric boot.ci" = "blue", "parametric" = "red"))
...
The relationship between number of geons and complexity rating is plotted below. Each point corresponds to an object item (8 per condition). The x-coordinates have been jittered to avoid over-plotting. The confidence intervals are calculated via non-parametric bootstrapping.
...
We also analyzed the log length of the production in characters as a function of study times (Study 9, right below). Length of production was correlated with study times: Longer labels were coined for objects that were studied longer (_r_=.16, _p_<.001). 
...
```{r, echo = F, fig.width = 4, fig.height = 4, warning = F, message = F}
if (processNorms) {
  # plot histogram
  sd = sd(d$log.rt)
...
p_syl = ggplot(ms.syl, aes(x=mrc.syl)) + 
  ...
  annotate("text", x=5, y=200, color = "red", size = rs,
...
246 participants completed the rating task. We excluded participants who missed a simple math problem in the middle of the task that served as an attentional check. This excluded 6 participants (2%). Complexity ratings were highly correlated with length. Below we plot complexity as a function of each of the three length metrics. Each point corresponds to a word. The x-coordinates have been jittered to limit over-plotting.
...
```{r, echo = F, fig.width = 8, fig.height = 6, include = F}
...
# partial out frequency 
ms.phon.conc <- aggregate(complexity ~ word + mrc.phon + mrc.conc + subt.log.freq, data=eng, mean) # do this again without freq because drops missing words
...
```{r 11:google_xling, echo = F, fig.width = 4, fig.height = 4}
...
We counted the number of unicode characters for each translation. Plotted below is the  mean number of characters for all 499 words for all 80 languages. The dashed line indicates the grand mean across languages (_M_ = 9.97). Red bars indicate languages where the accuracy was checked by a native speaker and pink bars indicate unchecked languages.
...
Variability in word length within languages was positively correlated with complexity ratings. Below the correlation coefficients are plotted for each language. Red bars indicate languages where the accuracy was checked by a native speaker and pink bars indicate unchecked languages. The dashed line indicates the grand mean correlation across languages. Triangles indicate the correlation between complexity and length, partialling out spoken frequency in English. Circles indicate the correlation between complexity and length for the subset of words that are monomorphemic in English. Squares indicate the correlation between complexity and length for the subset of open class words. 
...
Crump, M., McDonnell, J., & Gureckis, T.(2013). Evaluating Amazon's Mechanical Turk as a tool for experimental behavioral research. _PLoS ONE_, 8(3).

Sanchez-Meca J., Marin-Martinez, F., & Chacon-Moscoso, S. (2003). Effect-size indices for dichotomized outcomes
in meta-analysis. _Psychological Methods_, 8(4), 448-467.
