...
#png("figure/geons_plot2.png", height = 6, width = 12, units = "in", res=500)
...
#png("figure/obj_plot3.png", height = 6, width = 12, units = "in", res=500)
...
# get CIs
ms_open.phon$cih <- aggregate(complexity ~ word + mrc.phon, data=eng[eng$class != 0,], ci.high)$complexity
ms_open.phon$cil <- aggregate(complexity ~ word + mrc.phon, data=eng[eng$class != 0,], ci.low)$complexity
...
# get CIs
ms.phon.conc$cih <- aggregate(complexity ~ word + mrc.phon + mrc.conc, data=eng, ci.high)$complexity
ms.phon.conc$cil <- aggregate(complexity ~ word + mrc.phon + mrc.conc, data=eng, ci.low)$complexity
...
# get CIs
ms_mono.phon$cih <- aggregate(complexity ~ word + mrc.phon, data=eng[eng$clx.morph == 1,], ci.high)$complexity
ms_mono.phon$cil <- aggregate(complexity ~ word + mrc.phon, data=eng[eng$clx.morph == 1,], ci.low)$complexity
...
# get median split
ms.morph.conc$conc.split = ifelse(ms.morph.conc$mrc.conc < median(ms.morph.conc$mrc.conc), 1, 2)
...
# get median split
ms.syl.conc$conc.split = ifelse(ms.syl.conc$mrc.conc < median(ms.syl.conc$mrc.conc), 1, 2)
...
ms.freq.phon.complexity <- aggregate(complexity ~ word + subt.log.freq + mrc.phon, data=eng, mean) 
...
# mono-morphemic
m_syl_m = lm(mrc.syl ~ complexity + mrc.fam + mrc.imag + mrc.conc  + subt.log.freq, eng[eng$clx.morph == 1,])
m_phon_m = lm(mrc.phon ~ complexity + mrc.fam + mrc.imag + mrc.conc + subt.log.freq, eng[eng$clx.morph == 1,])
summary(m_syl_m)
summary(m_phon_m)

# open class
m_syl_o = lm(mrc.syl ~ complexity + mrc.fam + mrc.imag + mrc.conc  + subt.log.freq, eng[eng$class != 0,])
m_phon_o = lm(mrc.phon ~ complexity + mrc.fam + mrc.imag + mrc.conc  + subt.log.freq, eng[eng$class != 0,])
...
We translated all 499 words from Study 10 into 79 languages using Google translate (<a href="http://www.translate.google.com" target="_blank">www.translate.google.com</a>, retrieved March 2014). We translated the set of words into all languages available in Google translate. Words that were translated as English words were removed from the data set.
...
checksR = read.csv("data/corpus/translation_accuracy.csv")[1:499,]
...
qplot(accuracy.df$language, accuracy.df$accuracy, fill = I("red"),
      ...
      data=accuracy.df, geom="bar", ylab="Prop. correct", 
...
Brysbaert, M., & B. New (2009). Moving beyond Kucera and Francis: A critical evaluation of current word frequency norms and the introduction of a new and improved word frequency measure for American English. _Behavior Research Methods_, _41_, 977–990.

Crump M., J. McDonnell , & T. Gureckis,  (2013). Evaluating Amazon's Mechanical Turk as a Tool for Experimental Behavioral Research. _PLoS ONE_, 8(3).
