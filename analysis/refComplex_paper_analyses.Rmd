...
d$Answer.bet_complex <- as.numeric(str_replace_all(as.character(d$Answer.bet_complex),"\\\"",""))
d$Answer.bet_simple<- as.numeric(str_replace_all(as.character(d$Answer.bet_simple),"\\\"",""))
```

### *plot by condition*
```{r, warning = FALSE, message = FALSE, output = FALSE}
ms <- aggregate(Answer.bet_complex ~ Answer.condition, d, mean)
ci95 <- function(x) {sem(x)*1.96}
ms$err <- aggregate(Answer.bet_complex ~ Answer.condition, data=d, ci95)$Answer.bet_complex 
ms$n <- aggregate(workerid ~ Answer.condition, data=d, function(x) {length(unique(x))})$workerid
ms$syllables = c(2,4,6)

qplot(syllables,Answer.bet_complex, ylim=c(0,100),position=position_dodge(),
      data=ms,geom="line",ylab="Mean bet to complex object", xlab="Number of syllables")  +
  geom_linerange(aes(ymin=Answer.bet_complex-err,ymax=Answer.bet_complex+err), position=position_dodge(.9)) +
...
### relationship between bet to complex and number of syllables
```{r}
d$len <- 2
d$len[d$Answer.condition=='"4_syllables"'] <- 4
d$len[d$Answer.condition=='"6_syllables"'] <- 6

summary(lm(Answer.bet_complex~ len, data=d))
```
